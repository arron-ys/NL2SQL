...
DataTalk是一个企业级 AI 查数的服务，可以被上层的应用或Agent等调用，它把企业的业务、术语、口径收敛为可机读的语义层，再通过一套可控pipeline把自然语言转成SQL，最终返回基于企业真实数据的结构化结果 + 可解释回答。  

**核心价值：**
- **业务上**：让老板当场拍板、运营少做搬运、销售不再等人——**口径一致、权限可控、结果可追溯** 
- **技术上**：语义收敛 + 受控生成 + 确定性编译 —— 让 AI 输出从"涌现"变成"可治理"
...
| 传统 NL2SQL | DataTalk  | 解决的深层问题 |
| :--- | :--- | :--- |
| LLM 直接生成 SQL | LLM 只生成 **Plan**，SQL 由确定性编译器生成 | 消除 **LLM 幻觉**的系统性风险 |
| Prompt 里塞 schema 文档 | **语义层收敛** 为可检索的结构化定义 | **口径漂移问题** 从根源解决 |
| 事后审计日志 | 每个 Stage 输出 **结构化 artifact** | **可追溯性** 是设计时约束，不是运维补丁 |
| ACL 在应用层做 | RBAC **前置到检索阶段** (fail-closed) | **权限不是功能，是架构** |
| "能用就行" | 指标级别的评测体系 (EX/VES/RA/SR) | **可量化结果**，不是"感觉变好了" |
...
> 一句话提问 → 返回结构化结果 + 可解释依据（Trace / SQL 可追溯），并强制执行租户与角色权限（RLS）。

![Demo GIF](docs/assets/demo_execute.gif)
...
```json
{
    "answer_text": "公司总体销售额为 684.39 万元（精确值：6843930.28）。",
    "data_list": [
        {
            "sub_query_id": "req-20251228124833-b49c796b-000",
            "title": "查询公司的总体销售额",
            "data": {
                "columns": [
                    "METRIC_GMV"
